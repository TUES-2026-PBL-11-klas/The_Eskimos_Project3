...
resource "helm_release" "kube_prometheus_stack" {
  ...
  values = [yamlencode({
    # Grafana enabled — fits within the s-2vcpu-4gb node (~360m/1066Mi already in use; Grafana adds ~50m/128Mi).
    # persistence disabled: emptyDir avoids a second PVC; provisioned dashboards survive restarts via ConfigMaps.
    # Admin password auto-generated by the chart; retrieve with:
    ...
    grafana = {
      ...
      ingress = {
        ...
        hosts = ["grafana.eskimos.me"]
        tls   = [{ secretName = "grafana-tls", hosts = ["grafana.eskimos.me"] }]
      }
    }
